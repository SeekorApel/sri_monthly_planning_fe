...
import { IQuadrant} from 'src/app/response/quadrant';
...
export class QuadrantService {
  ...
  getAllQuadrant(): Observable<ApiResponse<IQuadrant[]>> {
    return this.http.get<ApiResponse<IQuadrant[]>>(`${this.baseUrl}/getAllQuadrant`);
  }
...
signIn(userName: string, password: string): Observable<{ data: string }> {
  ...
  saveQuadrantsExcelFile(formData: FormData): Observable<any> {
