...
export class ViewSizeComponent implements OnInit {
  ...
  constructor(
    ...
    private sizeService: SizeService // Inject PlantService
  ) { }
  ngOnInit(): void {
  }
...
this.sizeService.getAllSize().subscribe(
  (response) => {
    this.sizeList = response.data; // Simpan data plant ke dalam variabel
  },
  (error) => {
...
if (this.file) {
  ...
  this.sizeService.signIn('Aurel', 'polman').subscribe(
    (signinResponse) => {
        const token = signinResponse.data; 

      // Now upload the Excel file
      this.sizeService.savePlantsExcelFile(formData).subscribe(
        (response) => {
