...
export class ViewMachineCuringTypeComponent implements OnInit {
  ...
  constructor(
    ...
    private machineCuringTypeService: MachineCuringTypeService // Inject PlantService
  ) { }
  ...
  loadPlant() {
    this.machineCuringTypeService.getAllPlant().subscribe(
      (response) => {
  ...
  uploadExcelFile() {
    if (this.file) {
      ...
      this.machineCuringTypeService.signIn('Aurel', 'polman').subscribe(
        (signinResponse) => {
            const token = signinResponse.data; 
  
          // Now upload the Excel file
          this.machineCuringTypeService.savePlantsExcelFile(formData).subscribe(
            (response) => {
