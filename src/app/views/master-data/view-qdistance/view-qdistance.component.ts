...
export class ViewQDistanceComponent implements OnInit {
  ...
  constructor(
    ...
    private qDistanceService: QDistanceService // Inject PlantService
  ) { }
  ngOnInit(): void {
  }
...
loadQDistance() {
this.qDistanceService.getAllQDistance().subscribe(
  (response) => {
    this.qDistanceList = response.data; // Simpan data plant ke dalam variabel
  },
  (error) => {
...
if (this.file) {
  ...
  this.qDistanceService.signIn('Aurel', 'polman').subscribe(
    (signinResponse) => {
        const token = signinResponse.data; 

      // Now upload the Excel file
      this.qDistanceService.savePlantsExcelFile(formData).subscribe(
        (response) => {
